...
export async function generateStaticParams() {
  return getArticlesByCategory('govt').map((a) => ({ slug: a.slug }))
}

export default async function GovtArticlePage({ params }: Props) {
  const { slug } = await params
  const article = getArticleBySlug(slug)
  if (!article || article.category !== 'govt') notFound()

  const related = getArticlesByCategory('govt').filter(a => a.slug !== article.slug).slice(0, 3)
    ...
    <main className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-4 py-3 text-sm text-gray-500 flex items-center gap-2 flex-wrap">
          ...
          <span>/</span>
          <Link href="/govt" className="hover:text-green-700">সরকারি সেবা</Link>
          <span>/</span>
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <div className="lg:col-span-2">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="bg-blue-100 text-blue-700 text-xs font-medium px-3 py-1 rounded-full">সরকারি সেবা</span>
            ...
            {/* Article content — explicit styles, no typography plugin needed */}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-green-700 text-white rounded-2xl p-5">
              ...
              <p className="text-green-100 text-sm mb-4">EMI, DPS, আয়কর সহ বিভিন্ন আর্থিক হিসাব করুন।</p>
              <Link href="/tools" className="bg-white text-green-700 font-semibold text-sm px-4 py-2 rounded-lg hover:bg-green-50 transition-colors inline-block">
                সব টুলস দেখুন →
              </Link>
            </div>
          </div>
